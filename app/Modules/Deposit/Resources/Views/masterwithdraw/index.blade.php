...
@section('title', 'Master Withdraw list')
@section('page_title', 'Master Withdraw list')
@section('page_content')
                                ...
                                <div class="col-md-12">
                                    <div class="p-5">
                                        <table class="table table-hover dataTable table-striped w-full"
                                            ...
                                            <tbody>
                                            @if(count($masterWithdraws) > 0)
                                                @php($i=1)
                                                @foreach ($masterWithdraws as $masterWithdraw)
                                                    <tr>
                                                        <td>{{ $i++ }}</td>
                                                        <td>{{ $masterWithdraw->withdrawAmount }}</td>
                                                        <td> {{ $masterWithdraw->userCreated->userRole->name}} </td>
                                                        <td>
                                                            @if($masterWithdraw->created_at)
                                                                <?php
                                                                $db_time = DateTime::createFromFormat('Y-m-d H:i:s', $masterWithdraw->created_at, new DateTimeZone("UTC"));
    ...
    <div class="md-fab-wrapper branch-create">
        <a title="Create betoptions" id="add_branch_button" href="{{ route('master_withdraw_create') }}"
